...
export class TufteAstTransformer implements AstTransformer<AstRootNode> {
    /** @inheritdoc */
    public transform(ast: AstRootNode): AstRootNode {
        ...
        const withoutAbstract = this.removeAbstractNodes(withoutAuthor);
    ...
    private removeAbstractNodes(ast: AstRootNode): AstRootNode {
        let newComponentNodes = [ ...ast.v ];

        for (let i = 0, l = newComponentNodes.length; i < l; i++) {
            const componentNode = newComponentNodes[i];

            if (isAbstractHeadingNode(componentNode)) {
                const nextNode = newComponentNodes[i + 1];
                const isNextAuthorContent = nextNode && nextNode.t === Constants.NodeTypes.PARAGRAPH;
...
function isAbstractHeadingNode(node: AstBaseNode): boolean {
    return node.t === Constants.NodeTypes.HEADING &&
        (node as AstHeadingComponentNode).v.p.type <= 2 &&
        (node as AstHeadingComponentNode).v.v.toLowerCase().trim() === Constants.Keywords.ABSTRACT;
}
